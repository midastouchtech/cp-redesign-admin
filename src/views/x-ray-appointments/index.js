...
const Appointments = ({ socket }) => {
  ...
  const handleSearch = async () => {
    setLoading(true);
    setNotFound(false);
    socket.emit("SEARCH_APPOINTMENT", { term: searchTerm, type: "x-rays" });
  ...
  const getAllAppointments = () => {
    ...
    socket.emit("GET_ALL_APPOINTMENTS", { pageLimit, type: "x-rays" });
  ...
  const getNextMonthsAppointments = (p) => {
    socket.emit("GET_NEXT_MONTHS_APPOINTMENTS", { page: p, pageLimit, type: "x-rays" });
  ...
  const getPrevMonthsAppointments = (p) => {
    socket.emit("GET_PREVIOUS_MONTHS_APPOINTMENTS", { page: p, pageLimit, type: "x-rays" });
  ...
  const getPageAppointments = (p) => {
    if (monthType === "any") {
      socket.emit("GET_NEXT_PAGE_APPOINTMENTS", { page: p, pageLimit, type: "x-rays" });
    ...
    <div className="container-fluid">
      <div className="d-flex flex-wrap mb-2 align-items-center justify-content-between">
        <div className="mb-3 mr-3">
          <h6 className="fs-16 text-black font-w600 mb-0">X-Ray Appointments</h6>
          <span className="fs-14">On this page you will only be able find and view appointments that have been booked for x-rays </span>
