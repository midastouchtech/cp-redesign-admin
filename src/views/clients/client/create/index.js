...
function App({ socket }) {
  ...
  const saveUser = async () => {
    const control = blockingControl || await getBlockingPlatformControlOrFallback("block_new_signups");
    if (control) {
      alert(platformControlMessage(control, "New client creation is temporarily disabled."));
            ...
            <div className="card-body">
              {blockingControl && (
                <div className="alert alert-danger">
                  {platformControlMessage(blockingControl, "New client creation is temporarily disabled.")}
                </div>
              )}
              <button
